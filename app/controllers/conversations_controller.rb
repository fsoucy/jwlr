class ConversationsController < ApplicationController
  ...
  def pull_messages
    conversation = Conversation.find(params[:id])
    new_messages = conversation.messages.paginate(:page => params[:page], :per_page => 50).order("created_at DESC").pluck(:content, :sender_id).reverse
    data = Array.new
    new_messages.each do |message|
      data.append([message[0], User.find(message[1]).name])
    end
